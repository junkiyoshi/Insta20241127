...
void Actor::update(const int& frame_span, vector<glm::vec3>& location_list, vector<vector<int>>& next_index_list, vector<int>& destination_list) {
	...
	while (this->log.size() > frame_span * 2 ) { this->log.pop_back(); }
}
...
void Actor::draw() {
	...
	for (int i = 1; i < this->log.size(); i++) {
		...
		ofSetColor(ofColor(this->color, ofMap(i, 1, this->log.size(), 255, 0)));
...
void ofApp::setup() {
	...
	for (int x = -360; x <= 360; x += span) {
		...
		for (int y = -180; y <= 180; y += span) {
			...
			for (int z = -180; z <= 180; z += span) {
				...
				if (abs(x) > 240 || (abs(y) < 60 && abs(z) < 60)) {
	...
	for (int i = 0; i < 600; i++) {
		...
		color.setHsb(ofMap(i, 0, 600, 0, 255), 200, 255);
